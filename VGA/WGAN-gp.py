...
def computeGradientPenalty(discriminator, real, fake):
    alpha = torch.rand((real.shape[0], 1, 1, 1))
    interpolates = (alpha * real + ((1 - alpha) * fake)).requires_grad_(True)
    disc_interpolates = discriminator(interpolates)
    f = Tensor(real.shape[0], 1).fill_(1.0)
...
for epoch in range(200):
    ...
    for img, _ in (pbar := tqdm(dataloader)):
        ...
        fakeImg = generator(z)
        gradientPenalty = computeGradientPenalty(discriminator, img.data, fakeImg.data)

        lossD = -torch.mean(discriminator(img)) + torch.mean(discriminator(fakeImg)) + lambdaGp * gradientPenalty
